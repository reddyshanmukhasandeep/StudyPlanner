...
<p class="bottom30">
	This page displays all the terms.
	<br /><br />
...
<% // New term button at the top should be displayed only if the amount of results per page is > 10 %>
<c:if test="${resultList.pageSize > 10}">
	<a href='<s:url value="/staff/termcreate" />' class="btn btn-default bottom20">New Term</a>
</c:if>
...
<table class="table table-bordered table-striped table-responsive">
	...
	<tbody>
		<c:forEach items="${resultList.pageList}" var="term" varStatus="status">
			<tr>
				<td><c:out value="${resultList.pageSize * resultList.page + status.count}" /></td>
				<td>
					<a href='<s:url value="/staff/termedit/${term.year}/${term.number}" />'>
						<c:out value="${term.year}" />
					</a>
				</td>
				<td><c:out value="${term.number}" /></td>
				<td><c:out value="${term.startDate}" /></td>
				<td><c:out value="${term.finishDate}" /></td>
				<td>
					<c:choose>
						<c:when test="${term.enabled == 1}">Yes</c:when>
						<c:when test="${term.enabled == 0}">No</c:when>
					</c:choose>
				</td>
			</tr>
		</c:forEach>
			...
			<c:if test="${resultList.pageCount > 1}">
				<ul class="pagination">
					...
					<li<c:if test="${resultList.firstPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.firstPage}"><s:url value="/staff/termlist?page=${resultList.page-1}" /></c:when>
							</c:choose>
						">&laquo;</a>
					</li>
					...
					<c:if test="${resultList.pageCount > 5 && resultList.page > 2}">
						<li><a href='<s:url value="/staff/termlist?page=0" />'>1</a></li>
					...
					<c:forEach begin="0" end="${resultList.pageCount-1}" varStatus="pageNumber">
						<c:if test="${
							...
							<li<c:if test="${resultList.page == pageNumber.index}"> class="active"</c:if>>
								<a href="<s:url value="/staff/termlist?page=${pageNumber.index}" />">${pageNumber.index+1}</a>
							</li>
						</c:if>
					</c:forEach>
					...
					<c:if test="${resultList.page < resultList.pageCount - 3 &&	resultList.pageCount > 5}">
						...
						<li><a href='<s:url value="/staff/termlist?page=${resultList.pageCount-1}" />'>${resultList.pageCount}</a></li>
					</c:if>
					...
					<li<c:if test="${resultList.lastPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.lastPage}"><s:url value="/staff/termlist?page=${resultList.page+1}" /></c:when>
							</c:choose>
						">&raquo;</a>
					</li>
				</ul>
	...
	<script>
		...
		$(function() {
			$('#resultsPerPage').change(function(){
				...
				location.href = '<s:url value="/staff/termlist?pageSize=" />' + pageSize;
			});
		});
	</script>
</c:if>
