...
<p class="bottom30">
	This page displays all the courses available for enrollment.
	<br /><br />
			...
			<c:if test="${resultList.pageCount > 1}">
				<ul class="pagination">
					...
					<li<c:if test="${resultList.firstPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.firstPage}"><s:url value="/courses/list?page=${resultList.page-1}" /></c:when>
							</c:choose>
						">&laquo;</a>
					</li>
					...
					<c:if test="${resultList.pageCount > 5 && resultList.page > 2}">
						<li><a href='<s:url value="/courses/list?page=0" />'>1</a></li>
					...
					<c:forEach begin="0" end="${resultList.pageCount-1}" varStatus="pageNumber">
						<c:if test="${
							...
							<li<c:if test="${resultList.page == pageNumber.index}"> class="active"</c:if>>
								<a href="<s:url value="/courses/list?page=${pageNumber.index}" />">${pageNumber.index+1}</a>
							</li>
						</c:if>
					</c:forEach>
					...
					<c:if test="${resultList.page < resultList.pageCount - 3 &&	resultList.pageCount > 5}">
						...
						<c:if test="${resultList.pageCount > 5}">
							<li><a href='<s:url value="/courses/list?page=${resultList.pageCount-1}" />'>${resultList.pageCount}</a></li>
						</c:if>
					</c:if>
					...
					<li<c:if test="${resultList.lastPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.lastPage}"><s:url value="/courses/list?page=${resultList.page+1}" /></c:when>
							</c:choose>
						">&raquo;</a>
					</li>
				</ul>
	...
	<script>
		...
		$(function() {
			$('#resultsPerPage').change(function(){
				...
				location.href = '<s:url value="/courses/list?pageSize=" />' + pageSize;
			});
		});
	</script>
</c:if>
