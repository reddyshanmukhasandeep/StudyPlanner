...
<p class="bottom30">
	This page displays all the courses planned or enrolled.
	<br /><br />
...
<% // Info and error messages. Hidden by default %>
...
<% // New enrolment button at the top should be displayed only if the amount of results per page is > 10 %>
<c:if test="${resultList.pageSize > 10}">
	<a href='<s:url value="/student/enrollmentcreate" />' class="btn btn-default bottom20">Add Course</a>
</c:if>
	...
	<tbody>
		<c:forEach items="${resultList.pageList}" var="enrollment" varStatus="status">
			<tr>
				...
				<td>
					<a href='<s:url value="/student/enrollmentedit/${enrollment.id}" />'>
						<c:out value="${enrollment.courseAvailability.course.title}" />
					</a>
				</td>
			...
			<c:if test="${resultList.pageCount > 1}">
				<ul class="pagination">
					...
					<li<c:if test="${resultList.firstPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.firstPage}"><s:url value="/student/enrollmentlist?page=${resultList.page-1}" /></c:when>
							</c:choose>
						">&laquo;</a>
					</li>
					...
					<c:if test="${resultList.pageCount > 5 && resultList.page > 2}">
						<li><a href='<s:url value="/student/enrollmentlist?page=0" />'>1</a></li>
					...
					<c:forEach begin="0" end="${resultList.pageCount-1}" varStatus="pageNumber">
						<c:if test="${
							...
							<li<c:if test="${resultList.page == pageNumber.index}"> class="active"</c:if>>
								<a href="<s:url value="/student/enrollmentlist?page=${pageNumber.index}" />">${pageNumber.index+1}</a>
							</li>
						</c:if>
					</c:forEach>
					...
					<c:if test="${resultList.page < resultList.pageCount - 3 &&	resultList.pageCount > 5}">
						...
						<li><a href='<s:url value="/student/enrollmentlist?page=${resultList.pageCount-1}" />'>${resultList.pageCount}</a></li>
					</c:if>
					...
					<li<c:if test="${resultList.lastPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.lastPage}"><s:url value="/student/enrollmentlist?page=${resultList.page+1}" /></c:when>
							</c:choose>
						">&raquo;</a>
					</li>
				</ul>
	...
	<script>
		...
		$(function() {
			$('#resultsPerPage').change(function(){
				...
				location.href = '<s:url value="/student/enrollmentlist?pageSize=" />' + pageSize;
			});
		});
	</script>
</c:if>
