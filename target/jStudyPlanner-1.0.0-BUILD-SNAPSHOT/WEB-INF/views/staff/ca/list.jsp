...
<p class="bottom30">
	This page displays all the courses available for enrollment in various campuses.
	<br /><br />
...
<% // New course (and multiple) buttons at the top should be displayed only if the amount of results per page is > 10 %>
<c:if test="${resultList.pageSize > 10}">
	<div class="row bottom20">
		<a href='<s:url value="/staff/cacreate" />' class="btn btn-default">New Course</a>
		<a href='<s:url value="/staff/cacreatemultiple" />' class="btn btn-default left30">Add Multiple Courses</a>
	</div>
</c:if>
...
<table class="table table-bordered table-striped table-responsive">
	<thead>
		<tr>
			<th>#</th>
			<th>Campus</th>
			<th>Term (year, number)</th>
			<th>Course</th>
			<th>Enabled</th>
		</tr>
	</thead>
	<tbody>
		<c:forEach items="${resultList.pageList}" var="ca" varStatus="status">
			<tr>
				<td><c:out value="${resultList.pageSize * resultList.page + status.count}" /></td>
				<td><c:out value="${ca.campus.title}" /></td>
				<td><c:out value="${ca.term.year} ${ca.term.number}" /></td>
				<td>
					<a href='<s:url value="/staff/caedit/${ca.id}" />'>
						<c:out value="${ca.course.title}" />
					</a>
				</td>
				<td>
					<c:choose>
						<c:when test="${ca.enabled == 1}">Yes</c:when>
						<c:when test="${ca.enabled == 0}">No</c:when>
					</c:choose>
				</td>
			</tr>
		</c:forEach>
			...
			<c:if test="${resultList.pageCount > 1}">
				<ul class="pagination">
					...
					<li<c:if test="${resultList.firstPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.firstPage}"><s:url value="/staff/calist?page=${resultList.page-1}" /></c:when>
							</c:choose>
						">&laquo;</a>
					</li>
					...
					<c:if test="${resultList.pageCount > 5 && resultList.page > 2}">
						<li><a href='<s:url value="/staff/calist?page=0" />'>1</a></li>
					...
					<c:forEach begin="0" end="${resultList.pageCount-1}" varStatus="pageNumber">
						<c:if test="${
							...
							<li<c:if test="${resultList.page == pageNumber.index}"> class="active"</c:if>>
								<a href="<s:url value="/staff/calist?page=${pageNumber.index}" />">${pageNumber.index+1}</a>
							</li>
						</c:if>
					</c:forEach>
					...
					<c:if test="${resultList.page < resultList.pageCount - 3 &&	resultList.pageCount > 5}">
						...
						<li><a href='<s:url value="/staff/calist?page=${resultList.pageCount-1}" />'>${resultList.pageCount}</a></li>
					</c:if>
					...
					<li<c:if test="${resultList.lastPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.lastPage}"><s:url value="/staff/calist?page=${resultList.page+1}" /></c:when>
							</c:choose>
						">&raquo;</a>
					</li>
				</ul>
	...
	<script>
		...
		$(function() {
			$('#resultsPerPage').change(function(){
				...
				location.href = '<s:url value="/staff/calist?pageSize=" />' + pageSize;
			});
		});
	</script>
</c:if>
