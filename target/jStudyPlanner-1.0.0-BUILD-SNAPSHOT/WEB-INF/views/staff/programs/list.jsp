...
<p class="bottom30">
	This page displays all the programs.
	<br /><br />
...
<% // New program button at the top should be displayed only if the amount of results per page is > 10 %>
<c:if test="${resultList.pageSize > 10}">
	<a href='<s:url value="/staff/programcreate" />' class="btn btn-default bottom20">New Program</a>
</c:if>
	...
	<tbody>
		<c:forEach items="${resultList.pageList}" var="program" varStatus="status">
			<tr>
				...
				<td>
					<a href='<s:url value="/staff/programedit/${program.id}" />'>
				...
				<td>
					<c:choose>
						<c:when test="${program.enabled == 1}">Yes</c:when>
						<c:when test="${program.enabled == 0}">No</c:when>
					</c:choose>
				</td>
			</tr>
		</c:forEach>
			...
			<c:if test="${resultList.pageCount > 1}">
				<ul class="pagination">
					...
					<li<c:if test="${resultList.firstPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.firstPage}"><s:url value="/staff/programlist?page=${resultList.page-1}" /></c:when>
							</c:choose>
						">&laquo;</a>
					</li>
					...
					<c:if test="${resultList.pageCount > 5 && resultList.page > 2}">
						<li><a href='<s:url value="/staff/programlist?page=0" />'>1</a></li>
					...
					<c:forEach begin="0" end="${resultList.pageCount-1}" varStatus="pageNumber">
						<c:if test="${
							...
							<li<c:if test="${resultList.page == pageNumber.index}"> class="active"</c:if>>
								<a href="<s:url value="/staff/programlist?page=${pageNumber.index}" />">${pageNumber.index+1}</a>
							</li>
						</c:if>
					</c:forEach>
					...
					<c:if test="${resultList.page < resultList.pageCount - 3 &&	resultList.pageCount > 5}">
						...
						<li><a href='<s:url value="/staff/programlist?page=${resultList.pageCount-1}" />'>${resultList.pageCount}</a></li>
					</c:if>
					...
					<li<c:if test="${resultList.lastPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.lastPage}"><s:url value="/staff/programlist?page=${resultList.page+1}" /></c:when>
							</c:choose>
						">&raquo;</a>
					</li>
				</ul>
	...
	<script>
		...
		$(function() {
			$('#resultsPerPage').change(function(){
				...
				location.href = '<s:url value="/staff/programlist?pageSize=" />' + pageSize;
			});
		});
	</script>
</c:if>
...
<a href='<s:url value="/staff/programcreate" />' class="btn btn-default">New Program</a>