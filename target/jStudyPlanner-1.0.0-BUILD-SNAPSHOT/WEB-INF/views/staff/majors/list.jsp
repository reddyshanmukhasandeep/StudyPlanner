...
<p class="bottom30">
	This page displays all the majors.
	<br /><br />
...
<c:if test="${resultList.pageSize > 10}">
	<a href='<s:url value="/staff/majorcreate" />' class="btn btn-default bottom20">New Major</a>
</c:if>
...
<table class="table table-bordered table-striped table-responsive">
	...
	<tbody>
		<c:forEach items="${resultList.pageList}" var="major" varStatus="status">
			<tr>
				<td><c:out value="${resultList.pageSize * resultList.page + status.count}" /></td>
				<td><c:out value="${major.code}" /></td>
				<td>
					<a href='<s:url value="/staff/majoredit/${major.id}" />'>
				...
				<td>
					<c:choose>
						<c:when test="${major.enabled == 1}">Yes</c:when>
						<c:when test="${major.enabled == 0}">No</c:when>
					</c:choose>
				</td>
			</tr>
		</c:forEach>
			...
			<c:if test="${resultList.pageCount > 1}">
				<ul class="pagination">
					...
					<li<c:if test="${resultList.firstPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.firstPage}"><s:url value="/staff/majorlist?page=${resultList.page-1}" /></c:when>
							</c:choose>
						">&laquo;</a>
					</li>
					...
					<c:if test="${resultList.pageCount > 5 && resultList.page > 2}">
						<li><a href='<s:url value="/staff/majorlist?page=0" />'>1</a></li>
					...
					<c:forEach begin="0" end="${resultList.pageCount-1}" varStatus="pageNumber">
						<c:if test="${
							...
							<li<c:if test="${resultList.page == pageNumber.index}"> class="active"</c:if>>
								<a href="<s:url value="/staff/majorlist?page=${pageNumber.index}" />">${pageNumber.index+1}</a>
							</li>
						</c:if>
					</c:forEach>
					...
					<c:if test="${resultList.page < resultList.pageCount - 3 &&	resultList.pageCount > 5}">
						...
						<li><a href='<s:url value="/staff/majorlist?page=${resultList.pageCount-1}" />'>${resultList.pageCount}</a></li>
					</c:if>
					...
					<li<c:if test="${resultList.lastPage}"> class="disabled"</c:if>>
						<a href="
							<c:choose>
								...
								<c:when test="${! resultList.lastPage}"><s:url value="/staff/majorlist?page=${resultList.page+1}" /></c:when>
							</c:choose>
						">&raquo;</a>
					</li>
				</ul>
	...
	<script>
		...
		$(function() {
			$('#resultsPerPage').change(function(){
				...
				location.href = '<s:url value="/staff/majorlist?pageSize=" />' + pageSize;
			});
		});
	</script>
</c:if>
